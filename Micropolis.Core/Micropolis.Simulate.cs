...
public partial class Micropolis
{
    ...
    public void SimFrame()
    {
        ...
        if (++SpeedCycle > 1023) SpeedCycle = 0;

        if (SimSpeed == 1 && SpeedCycle % 5 != 0) return;

        if (SimSpeed == 2 && SpeedCycle % 3 != 0) return;
    ...
    public void Simulate()
    {
        ...
        var speedIndex = Utilities.Restrict((short)(SimSpeed - 1), (short)0, (short)2);
        ...
        switch (PhaseCycle)
        {
            case 0:
                ...
                if (++SimCycle > 1023) SimCycle = 0; // This is cosmic!
                ...
                if (!(SimCycle & 1).IsTrue()) SetValves();
            ...
            case 9:
                if (CityTime % Constants.CensusFrequency10 == 0) Take10Census();

                if (CityTime % Constants.CensusFrequency120 == 0) Take120Census();
            ...
            case 10:
                ...
                if (!(SimCycle % 5).IsTrue()) DecRateOfGrowthMap();
            ...
            case 13:
                ...
                if (SimCycle % speedCrimeScan[speedIndex] == 0) CrimeScan();

                break;

            case 14:

                if (SimCycle % speedPopulationDensityScan[speedIndex] == 0) PopulationDensityScan();

                break;

            case 15:

                if (SimCycle % speedFireAnalysis[speedIndex] == 0) FireAnalysis();
    ...
    public void DoNilPower()
    {
        ...
        {
            var z = Map[x, y];
            if ((z & (ushort)MapTileBits.CenterOfZone).IsTrue()) SetZonePower(new Position(x, y));
        }
    }
    ...
    public void DecRateOfGrowthMap()
    {
        ...
        for (y = 0; y < RateOfGrowthMap.height; y++)
        {
            ...
            {
                z--;
                z = Utilities.Restrict(z, -200, 200);
    ...
    public void SimLoadInit()
    {
        ...
        if (!ExternalMarket.IsTrue()) ExternalMarket = 4.0f;

        // Set game level
        if (GameLevel > Levels.Last || GameLevel < Levels.First) GameLevel = Levels.First;
        ...
        if (Scenario >= Scenario.Count) Scenario = Scenario.None;
    ...
    public void SetValves()
    {
        ...
        short resPopDenom = 8;
        var birthRate = 0.02f;
        var laborBaseMax = 1.3f;
        var internalMarketDenom = 3.7f;
        var projectedIndPopMin = 5.0f;
        var resRatioDefault = 1.3f;
        float resRatioMax = 2;
        float comRatioMax = 2;
        float indRatioMax = 2;
        short taxMax = 20;
        ...
        normalizedResPop = ResPop / (float)resPopDenom;
        ...
        internalMarket = (normalizedResPop + ComPop + IndPop) / internalMarketDenom;
        ...
        if (normalizedResPop > 0)
            resRatio = projectedResPop / normalizedResPop; // projected -vs- actual.
        else
        ...
        ResValve = (short)Utilities.Restrict(ResValve + (short)resRatio, -Constants.ResValveRange,
            Constants.ResValveRange);
        ComValve = (short)Utilities.Restrict(ComValve + (short)comRatio, -Constants.ComValveRange,
            Constants.ComValveRange);
        IndValve = (short)Utilities.Restrict(IndValve + (short)indRatio, -Constants.IndValveRange,
            Constants.IndValveRange);

        if (ResCap && ResValve > 0) ResValve = 0; // Need a stadium, so cap resValve.

        if (ComCap && ComValve > 0) ComValve = 0; // Need a airport, so cap comValve.

        if (IndCap && IndValve > 0) IndValve = 0; // Need an seaport, so cap indValve.
    ...
    public void Take10Census()
    {
        ...
        MoneyHist[0] = (short)Utilities.Restrict(x, 0, 255);

        ChangeCensus();

        var resPopScaled = ResPop >> 8;

        if (HospitalPop < resPopScaled) NeedHospital = 1;

        if (HospitalPop > resPopScaled) NeedHospital = -1;

        if (HospitalPop == resPopScaled) NeedHospital = 0;

        var faithfulPop = resPopScaled + Faith;

        if (ChurchPop < faithfulPop) NeedChurch = 1;

        if (ChurchPop > faithfulPop) NeedChurch = -1;

        if (ChurchPop == faithfulPop) NeedChurch = 0;
    }
    ...
    public void UpdateFundEffects()
    {
        ...
        if (RoadFund > 0)
            ...
            RoadEffect = (short)(RoadEffect * (float)RoadSpend / RoadFund);

        // Compute police station effects of funding
        PoliceEffect = Constants.MaxPoliceStationEffect;
        if (PoliceFund > 0)
            // Multiply with funding fraction
            PoliceEffect = (short)(PoliceEffect * (float)PoliceSpend / PoliceFund);

        // Compute fire station effects of funding
        FireEffect = Constants.MaxFireStationEffect;
        if (FireFund > 0)
            // Multiply with funding fraction
            FireEffect = (short)(FireEffect * (float)FireSpend / FireFund);
    ...
    public void mapScan(int x1, int x2)
    {
        ...
        {
            var mapVal = Map[x, y];
            if (mapVal == (ushort)MapTileCharacters.DIRT) continue;

            var tile = (ushort)(mapVal & (ushort)MapTileBits.LowMask); /* Mask off status bits  */

            if (tile < (ushort)MapTileCharacters.FLOOD) continue;
            ...
            if (tile < (ushort)MapTileCharacters.ROADBASE)
            {
                ...
                {
                    FirePop++;
                    if (!(GetRandom16() & 3).IsTrue()) DoFire(pos); /* 1 in 4 times */
                    continue;
                }
        ...
        if (RoadEffect < 15 * Constants.MaxRoadEffect / 16)
            ...
            if (!(GetRandom16() & 511).IsTrue())
            {
                ...
                if (!(curValue & (ushort)MapTileBits.Conductivity).IsTrue())
                {
                    ...
                    {
                        var tile = (ushort)(curValue & (ushort)MapTileBits.LowMask);
    ...
    {
        if ((GetRandom16() & 4095) == 0) Map[pos.X, pos.Y] = (ushort)MapTileCharacters.DIRT; /* Radioactive decay */
    }
    ...
    public void DoRoad(Position pos)
    {
        int tden, z;
        short[] densityTable =
            { (short)MapTileCharacters.ROADBASE, (short)MapTileCharacters.LTRFBASE, (short)MapTileCharacters.HTRFBASE };
        ...
        if ((mapValue & (ushort)MapTileBits.Burnable) == 0)
        {
            ...
            if (DoBridge(new Position(pos.X, pos.Y), tile)) return;
        }
        ...
        var trafficDensity = TrafficDensityMap.WorldGet(pos.X, pos.Y) >> 6;

        if (trafficDensity > 1) trafficDensity--;
        ...
        if (tden != trafficDensity)
        {
            ...
            if (trafficDensity > 0) z |= (ushort)MapTileBits.Animated;
    ...
    public bool DoBridge(Position pos, ushort tile)
    {
        ...
        short[] HBRTAB =
        {
            ...
            (short)MapTileCharacters.HBRDG0 | (short)MapTileBits.Bulldozable,
        ...
        short[] VBRTAB =
        {
            ...
            (short)MapTileCharacters.BRWV | (short)MapTileBits.Bulldozable,
    ...
    public void DoFire(Position pos)
    {
        ...
        for (z = 0; z < 4; z++)
            if ((GetRandom16() & 7) == 0)
            {
                ...
                {
                    var c = Map[xTem, yTem];
                    if (!(c & (ushort)MapTileBits.Burnable).IsTrue()) continue;
        ...
        if (GetRandom((short)rate) == 0) Map[pos.X, pos.Y] = RandomRubble();
    }
    ...
    public void FireZone(Position pos, ushort ch)
    {
        ...
        for (short y = -1; y < XYmax; y++)
        {
            ...
            if (!Position.TestBounds(xTem, yTem)) continue;
    ...
    public void RepairZone(Position pos, ushort zCent, short zSize)
    {
        ...
        for (short x = -1; x < zSize - 1; x++)
        {
            ...
            {
                var mapValue = Map[xx, yy];

                if ((mapValue & (ushort)MapTileBits.CenterOfZone).IsTrue()) continue;

                if ((mapValue & (ushort)MapTileBits.Animated).IsTrue()) continue;
    ...
    public void DoSpecialZone(Position pos, bool powerOn)
    {
        ...
        switch ((MapTileCharacters)tile)
        {
            case MapTileCharacters.POWERPLANT:
                ...
                if ((CityTime & 7) == 0) RepairZone(pos, (ushort)MapTileCharacters.POWERPLANT, 4); /* post */
            ...
            case MapTileCharacters.NUCLEAR:
                ...
                if ((CityTime & 7) == 0) RepairZone(pos, (ushort)MapTileCharacters.NUCLEAR, 4); /* post */
            ...
            case MapTileCharacters.FIRESTATION:
            {
                ...
                if (!(CityTime & 7).IsTrue()) RepairZone(pos, (ushort)MapTileCharacters.FIRESTATION, 3); /* post */
                ...
                var pos2 = new Position(pos);
                var foundRoad = FindPerimeterRoad(pos2);

                if (!foundRoad) z = z / 2; /* post FD's need roads  */
            ...
            case MapTileCharacters.POLICESTATION:
            {
                ...
                if (!(CityTime & 7).IsTrue()) RepairZone(pos, (ushort)MapTileCharacters.POLICESTATION, 3); /* post */
                ...
                if (!foundRoad) z = z / 2; /* post PD's need roads */
            ...
            case MapTileCharacters.STADIUM: // Empty stadium
                ...
                if (!(CityTime & 15).IsTrue()) RepairZone(pos, (ushort)MapTileCharacters.STADIUM, 4);
                ...
                if (powerOn)
                    ...
                    if (((CityTime + pos.X + pos.Y) & 31) == 0)
                    {
                        ...
                        Map[pos.X + 1, pos.Y] =
                            (ushort)(MapTileCharacters.FOOTBALLGAME1 + (ushort)MapTileBits.Animated);
                        Map[pos.X + 1, pos.Y + 1] =
                            (ushort)(MapTileCharacters.FOOTBALLGAME2 + (ushort)MapTileBits.Animated);
                    }
            ...
            case MapTileCharacters.AIRPORT:
                ...
                if ((CityTime & 7) == 0) RepairZone(pos, (ushort)MapTileCharacters.AIRPORT, 6);

                // If powered, display a rotating radar
                if (powerOn)
                {
                    if ((Map[pos.X + 1, pos.Y - 1] & (ushort)MapTileBits.LowMask) == (ushort)MapTileCharacters.RADAR)
                        Map[pos.X + 1, pos.Y - 1] = (ushort)(MapTileCharacters.RADAR0 + (ushort)MapTileBits.Animated +
                                                             (ushort)MapTileBits.Conductivity +
                                                             (ushort)MapTileBits.Burnable);
                }
                else
                {
                    Map[pos.X + 1, pos.Y - 1] = (ushort)(MapTileCharacters.RADAR + (ushort)MapTileBits.Conductivity +
                                                         (ushort)MapTileBits.Burnable);
                }
            ...
            case MapTileCharacters.PORT:
                ...
                if ((CityTime & 15) == 0) RepairZone(pos, (ushort)MapTileCharacters.PORT, 4);

                // If port has power and there is no ship, generate one
                if (powerOn && GetSprite(SpriteType.Ship) == null) GenerateShip();
    ...
    public void DoMeltdown(Position pos)
    {
        ...
        for (var y = pos.Y - 1; y < pos.Y + 3; y++)
            Map[x, y] = RandomFire();

        // Add lots of radiation tiles around the plant
        for (var z = 0; z < 200; z++)
        {
            var x = pos.X - 20 + GetRandom(40);
            var y = pos.Y - 15 + GetRandom(30);
            ...
            if ((t & (ushort)MapTileBits.CenterOfZone).IsTrue()) continue; // Ignore zones
    ...
    {
        return (ushort)((ushort)((ushort)MapTileCharacters.RUBBLE + (GetRandom16() & 7)) |
