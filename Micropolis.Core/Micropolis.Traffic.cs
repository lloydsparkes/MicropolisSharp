...
public partial class Micropolis
{
    ...
    public bool TryDrive(Position startPos, ZoneType dest)
    {
        ...
        {
            var dir = TryGo(drivePos, dirLast);
            if (dir != Direction.Invalid)
            {
                 ...
                 */
                if ((dist & 1).IsTrue()) PushPos(drivePos);
    ...
    /// <returns>
    ///     The tile in the indicated direction. If tile is off-world or an incorrect direction is given, \c DIRT is
    ///     returned.
    /// </returns>
    public ushort GetTileFromMap(Position pos, Direction dir, ushort defaultTile)
    {
        switch (dir)
        {
            case Direction.North:
                if (pos.Y > 0) return (ushort)(Map[pos.X, pos.Y - 1] & (ushort)MapTileBits.LowMask);
            ...
            case Direction.West:
                if (pos.X > 0) return (ushort)(Map[pos.X - 1, pos.Y] & (ushort)MapTileBits.LowMask);
    ...
    public bool DriveDone(Position pos, ZoneType destZone)
    {
        ...
        ushort[] targetLow =
            { (ushort)MapTileCharacters.COMBASE, (ushort)MapTileCharacters.LHTHR, (ushort)MapTileCharacters.LHTHR };
        ushort[] targetHigh =
            { (ushort)MapTileCharacters.NUCLEAR, (ushort)MapTileCharacters.PORT, (ushort)MapTileCharacters.COMBASE };

        //TODO: asserts fix & reenable
        //assert(ZT_NUM_DESTINATIONS == LENGTH_OF(targetLow));
        //assert(ZT_NUM_DESTINATIONS == LENGTH_OF(targetHigh));

        var l = targetLow[(int)destZone]; // Lowest acceptable tile value
        var h = targetHigh[(int)destZone]; // Highest acceptable tile value

        if (pos.Y > 0)
        {
            var z = (ushort)(Map[pos.X, pos.Y - 1] & (ushort)MapTileBits.LowMask);
            if (z >= l && z <= h) return true;
        }

        if (pos.X < Constants.WorldWidth - 1)
        {
            var z = (ushort)(Map[pos.X + 1, pos.Y] & (ushort)MapTileBits.LowMask);
            if (z >= l && z <= h) return true;
        }
    ...
    public bool RoadTest(ushort mapValue)
    {
        ...
        if (tile < (ushort)MapTileCharacters.ROADBASE || tile > (ushort)MapTileCharacters.LASTRAIL) return false;

        if (tile >= (ushort)MapTileCharacters.POWERBASE && tile < (ushort)MapTileCharacters.LASTPOWER) return false;
