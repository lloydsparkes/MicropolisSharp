...
public partial class Micropolis
{
    private static readonly ushort[] IdArray =
    {
        ...
        (ushort)MapTileCharacters.FLOOD, (ushort)MapTileCharacters.RADTILE, (ushort)MapTileCharacters.FIRE,
        (ushort)MapTileCharacters.ROADBASE,
        (ushort)MapTileCharacters.POWERBASE, (ushort)MapTileCharacters.RAILBASE, (ushort)MapTileCharacters.RESBASE,
        (ushort)MapTileCharacters.COMBASE,
        (ushort)MapTileCharacters.INDBASE, (ushort)MapTileCharacters.PORTBASE, (ushort)MapTileCharacters.AIRPORTBASE,
        (ushort)MapTileCharacters.COALBASE,
        (ushort)MapTileCharacters.FIRESTBASE, (ushort)MapTileCharacters.POLICESTBASE,
        (ushort)MapTileCharacters.STADIUMBASE, (ushort)MapTileCharacters.NUCLEARBASE,
        // FIXME: I think HBRDG_END should be HBRDG0...?
        (ushort)MapTileCharacters.HBRDG0, (ushort)MapTileCharacters.RADAR0, (ushort)MapTileCharacters.FOUNTAIN,
        (ushort)MapTileCharacters.INDBASE2,
    ...
    {
        var value = GetRandom(4);
        var tile = (ushort)(MapTileBits.Burnable | MapTileBits.Bulldozable);

        if (value == 4)
            tile |= (ushort)MapTileCharacters.FOUNTAIN | (ushort)MapTileBits.Animated;
        else
            tile |= (ushort)(value + (ushort)MapTileCharacters.WOODS2);

        if (effects.GetMapValue(x, y) != (ushort)MapTileCharacters.DIRT) return ToolResult.NeedBulldoze;
    ...
    {
        var tile = effects.GetMapTile(x, y);
    ...
    public ToolResult PutDownForest(int x, int y, ToolEffects effects)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            var xx = x + dx[i];
            var yy = y + dy[i];
            if (new Position(xx, yy).TestBounds()) SmoothTreesAt(xx, yy, true, effects);
        }
    ...
    {
        return (tileValue >= (ushort)MapTileCharacters.FIRSTRIVEDGE &&
                tileValue <= (ushort)MapTileCharacters.LASTRUBBLE) ||
               (tileValue >= (ushort)MapTileCharacters.POWERBASE + 2 &&
                tileValue <= (ushort)MapTileCharacters.POWERBASE + 12) ||
               (tileValue >= (ushort)MapTileCharacters.TINYEXP &&
                tileValue <= (ushort)MapTileCharacters.LASTTINYEXP + 2);
    }
    ...
    public int CheckSize(ushort tileValue)
    {
        ...
        if ((tileValue >= (ushort)MapTileCharacters.RESBASE - 1 &&
             tileValue <= (ushort)MapTileCharacters.PORTBASE - 1) ||
            (tileValue >= (ushort)MapTileCharacters.LASTPOWERPLANT + 1 &&
             tileValue <= (ushort)MapTileCharacters.POLICESTATION + 4) ||
        ...
        if ((tileValue >= (ushort)MapTileCharacters.PORTBASE && tileValue <= (ushort)MapTileCharacters.LASTPORT) ||
            (tileValue >= (ushort)MapTileCharacters.COALBASE &&
             tileValue <= (ushort)MapTileCharacters.LASTPOWERPLANT) ||
    ...
    public void CheckBorder(int xMap, int yMap, int sizeX, int sizeY, ToolEffects effects)
    {
        ...
        for (cnt = 0; cnt < sizeX; cnt++) ConnectTile(xMap + cnt, yMap - 1, ConnectTileCommand.Fix, effects);

        /* this will do the left bordering row */
        for (cnt = 0; cnt < sizeY; cnt++) ConnectTile(xMap - 1, yMap + cnt, ConnectTileCommand.Fix, effects);

        /* this will do the bottom bordering row */
        for (cnt = 0; cnt < sizeX; cnt++) ConnectTile(xMap + cnt, yMap + sizeY, ConnectTileCommand.Fix, effects);

        /* this will do the right bordering row */
        for (cnt = 0; cnt < sizeY; cnt++) ConnectTile(xMap + sizeX, yMap + cnt, ConnectTileCommand.Fix, effects);
    }
    ...
    /// <param name="effects">Storage of the effects.</param>
    public void PutBuilding(int leftX, int topY, int sizeX, int sizeY, ushort baseTile, bool aniFlag,
        ToolEffects effects)
    {
        for (var dy = 0; dy < sizeY; dy++)
        {
            ...
            for (var dx = 0; dx < sizeX; dx++)
            {
                ...
                var tileValue = (ushort)(baseTile | (ushort)MapTileBits.BurnableOrConductive);
                if (dx == 1)
                {
                    if (dy == 1)
                        tileValue |= (ushort)MapTileBits.CenterOfZone;
                    else if (dy == 2 && aniFlag) tileValue |= (ushort)MapTileBits.Animated;
                }
    ...
    public ToolResult PrepareBuildingSite(int leftX, int topY, int sizeX, int sizeY, ToolEffects effects)
    {
        ...
        if (leftX < 0 || leftX + sizeX > Constants.WorldWidth) return ToolResult.Failed;
        if (topY < 0 || topY + sizeY > Constants.WorldHeight) return ToolResult.Failed;
        ...
        for (var dy = 0; dy < sizeY; dy++)
        {
            ...
            for (var dx = 0; dx < sizeX; dx++)
            {
                ...
                var tileValue = effects.GetMapTile(posX, posY);
    ...
    {
        mapH--;
        mapV--; // Move position to top-left

        var prepareResult = PrepareBuildingSite(mapH, mapV,
            buildingProps.SizeX,
            buildingProps.SizeY,
            effects);
        if (prepareResult != ToolResult.Ok) return prepareResult;
    ...
    public void DoZoneStatus(short mapH, short mapV)
    {
        ...
        var tileNum = (ushort)(Map[mapH, mapV] & (ushort)MapTileBits.LowMask);
        ...
        if (i < 1 || i > 28) i = 28; // This breaks the program (when you click 'dirt')
        ...
        for (i = 0; i < 5; i++)
        {
            var id = Utilities.Restrict(GetDensity(i, mapH, mapV) + 1, 1, 20);
            status[i] = id;
        }
    ...
    public void DoShowZoneStatus(
        ...
        int x, int y)
    {
        Callback("showZoneStatus", "dddddddd", tileCategory.ToString(), s0.ToString(), s1.ToString(), s2.ToString(),
            s3.ToString(), s4.ToString(), x.ToString(), y.ToString());
    }
    ...
    public void PutRubble(int x, int y, int size, ToolEffects effects)
    {
        ...
        for (var yy = y; yy < y + size; yy++)
            ...
            {
                var tile = effects.GetMapTile(xx, yy);

                if (tile != (ushort)MapTileCharacters.RADTILE && tile != (ushort)MapTileCharacters.DIRT)
                {
                    tile = (ushort)(DoAnimation
                        ? (ushort)MapTileCharacters.TINYEXP + GetRandom(2)
                        : (ushort)MapTileCharacters.SOMETINYEXP);
                    effects.SetMapValue(xx, yy,
                        (ushort)(tile | (ushort)MapTileBits.Animated | (ushort)MapTileBits.Bulldozable));
                }
            }
    }
    ...
    {
        Callback("didTool", "sdd", name, x.ToString(), y.ToString());
    }
    ...
    {
        if (!new Position(x, y).TestBounds()) return ToolResult.Failed;
    ...
    public ToolResult BulldozerTool(short x, short y)
    {
        ...
        var result = BulldozerTool(x, y, effects);

        if (result == ToolResult.Ok) effects.ModifyWorld();
    ...
    public ToolResult BulldozerTool(short x, short y, ToolEffects effects)
    {
        ...
        var mapVal = effects.GetMapValue(x, y);
        var tile = (ushort)(mapVal & (ushort)MapTileBits.LowMask);

        var zoneSize = 0; // size of the zone, 0 means invalid.
        var deltaH = 0; // Amount of horizontal shift to the center tile of the zone.
        var deltaV = 0; // Amount of vertical shift to the center tile of the zone.
        ...
        if (zoneSize > 0)
        {
            ...
            int dozeX = x;
            int dozeY = y;
            var centerX = x + deltaH;
            var centerY = y + deltaV;
        ...
        if (tile == (ushort)MapTileCharacters.RIVER || tile == (ushort)MapTileCharacters.REDGE ||
            tile == (ushort)MapTileCharacters.CHANNEL)
        {
            result = ConnectTile(x, y, ConnectTileCommand.Bulldoze, effects);

            if (tile != effects.GetMapTile(x, y)) effects.AddCost(5);
        }
        else
        {
            result = ConnectTile(x, y, ConnectTileCommand.Bulldoze, effects);
    ...
    public ToolResult RoadTool(short x, short y, ToolEffects effects)
    {
        ...
        var result = ConnectTile(x, y, ConnectTileCommand.Road, effects);
    ...
    {
        if (!Position.TestBounds(x, y)) return ToolResult.Failed;

        var result = ConnectTile(x, y, ConnectTileCommand.RailRoad, effects);
    ...
    public ToolResult WireTool(short x, short y, ToolEffects effects)
    {
        ...
        var result = ConnectTile(x, y, ConnectTileCommand.Wire, effects);
    ...
    public ToolResult ParkTool(short x, short y, ToolEffects effects)
    {
        ...
        var result = PutDownPark(x, y, effects);
    ...
    public ToolResult BuildBuildingTool(short x, short y,
        ...
        ToolEffects effects)
    {
        var result = BuildBuilding(x, y, buildingProps, effects);
    ...
    {
        if (!Position.TestBounds(x, y)) return ToolResult.Ok;

        var result = PutDownNetwork(x, y, effects);
    ...
    public ToolResult WaterTool(short x, short y, ToolEffects effects)
    {
        ...
        if (result == ToolResult.Ok) result = PutDownWater(x, y, effects);
    ...
    public ToolResult ForestTool(short x, short y, ToolEffects effects)
    {
        ...
        var tile = effects.GetMapValue(x, y);

        if (IsTree(tile)) return ToolResult.Ok;
    ...
    {
        var result = DoTool(tool, tileX, tileY);
        ...
        short fromX, short fromY, short toX, short toY)
    {
        ...
        var dirX = toX > fromX ? (short)1 : (short)-1; // Horizontal step direction.
        var dirY = toY > fromY ? (short)1 : (short)-1; // Vertical step direction.
        ...
        var dx = (short)Math.Abs(fromX - toX); // number of horizontal steps.
        var dy = (short)Math.Abs(fromY - toY); // number of vertical steps.

        short subX = 0; // Each X step is dy sub-steps.
        short subY = 0; // Each Y step is dx sub-steps.
        var numSubsteps = Math.Min(dx, dy); // Number of sub-steps we can do.
