...
public partial class Micropolis
{
    ...
    public void CityEvaluation()
    {
        ...
        {
            var problemTable =
                new int[(int)CityVotingProblems
                    .NumberOfProblems]; // Score for each problem, higher the more severe the problem is.
            for (var z = 0; z < (int)CityVotingProblems.NumberOfProblems; z++) problemTable[z] = 0;
    ...
    public void EvalInit()
    {
        ...
        CityScore = 500;
        CityScoreDelta = 0;
        for (var i = 0; i < (int)CityVotingProblems.NumberOfProblems; i++) ProblemVotes[i] = 0;
        for (var i = 0; i < (int)CityVotingProblems.CountOfProblemsToComplainAbout; i++)
    ...
    {
        if (i < 0 || i >= (int)CityVotingProblems.CountOfProblemsToComplainAbout ||
            ProblemOrder[i] == (int)CityVotingProblems.NumberOfProblems)
            return -1;
    ...
    {
        var oldCityPop = CityPopulation;

        CityPopulation = GetPopulation();

        if (oldCityPop == -1) oldCityPop = CityPopulation;
    ...
    {
        var pop = (ResPop + (ComPop + IndPop) * 8L) * 20L;
        return pop;
    }
    ...
    {
        var cityClassification = CityClassification.Village;

        if (cityPopulation > 2000) cityClassification = CityClassification.Town;
        if (cityPopulation > 10000) cityClassification = CityClassification.City;
        if (cityPopulation > 50000) cityClassification = CityClassification.Capital;
        if (cityPopulation > 100000) cityClassification = CityClassification.Metropolis;
        if (cityPopulation > 500000) cityClassification = CityClassification.Megalopolis;
    ...
    {
        var problemTaken = new bool[(int)CityVotingProblems.NumberOfProblems]; // Which problems are taken?

        for (var z = 0; z < (int)CityVotingProblems.NumberOfProblems; z++)
        {
        ...
        for (var z = 0; z < (int)CityVotingProblems.CountOfProblemsToComplainAbout; z++)
        {
            // Find biggest problem not taken yet
            var maxVotes = 0;
            var bestProblem = (int)CityVotingProblems.CountOfProblemsToComplainAbout;
            for (var i = 0; i < (int)CityVotingProblems.CountOfProblemsToComplainAbout; i++)
            ...
            if (bestProblem < (int)CityVotingProblems.CountOfProblemsToComplainAbout) problemTaken[bestProblem] = true;
    ...
    {
        for (var z = 0; z < (int)CityVotingProblems.NumberOfProblems; z++) ProblemVotes[z] = 0;

        var problem = 0; // Problem to vote for
        var voteCount = 0; // Number of votes
        var loopCount = 0; // Number of attempts
        while (voteCount < 100 && loopCount < 600)
        {
            ...
            problem++;
            if (problem > (int)CityVotingProblems.NumberOfProblems - 1) problem = 0;
            loopCount++;
        }
    }
    ...
    private void GetScore(int[] problemTable)
    {
        ...
        for (z = 0; z < (int)CityVotingProblems.NumberOfProblems; z++) x += problemTable[z]; /* add 7 probs */
        ...
        if (ResCap) z = (int)(z * .85);

        if (ComCap) z = (int)(z * .85);

        if (IndCap) z = (int)(z * .85);

        if (RoadEffect < Constants.MaxRoadEffect) z -= (int)(Constants.MaxRoadEffect - RoadEffect);
        ...
        if (ResValve < -1000) z = (int)(z * .85);

        if (ComValve < -1000) z = (int)(z * .85);
